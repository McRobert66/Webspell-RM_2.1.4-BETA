    ...
    if($cal - $filesgranted == '0') {
      ...
      if(file_exists('../install.php')) {
        ...
        if($wsinstall == '1') {
          ...
          // Löscht die fonts.zip Datei vom Server
          $datei = '../install.php';

          if ( @ unlink ( $datei ) )
          {
            $wsinstallcompletedel = '<div class="alert alert-success" role="alert">Die Datei ' . $datei . ' wurde erfolgreich gelöscht!</div>';
          }
          else
          {
            $wsinstallcompletedel = '<div class="alert alert-danger" role="alert">Konnte die Datei ' . $datei . ' nicht löschen!</div>';
          }
